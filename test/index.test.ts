...
describe(`Japer`, () => {
  ...
  storeNames.forEach(storeName => {
    ...
    describe(`with store ${storeName}`, () => {
      ...
      it('should filter the documents', async () => {
        const loops = 3
        const postResults = [], results = []

        if (storeName === 'memory') {
          store.setFilter((envelope: Envelope): boolean => {
        ...
        for (let i = 0; i < loops; i++) {
          const type = i % 3 ? 'bad' : 'good'
          const result = await request(app)
            .post(`${url}/doc/document`)
            .send({ document:{ type: type, a: i, b: i + 1 }})
            .expect(200)
          postResults.push(result)
        }
        ...
        expect(results.length).toBe(Math.floor((loops - 1) / 3) + 1)

        const allDocsResult = await request(app)
          .get(`${url}/doc/document`)
          .expect(200)

        expect(allDocsResult.body.length).toBe(Math.floor((loops - 1) / 3) + 1)

        allDocsResult.body.forEach((envelope: Envelope) => expect(envelope.document.type).toBe('good'))
      })
